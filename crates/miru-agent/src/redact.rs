...
//! |-------------------|-----------------|-------------------------------------|
//! | KeyType           | text            | length + SHA-256 digest             |
...
/// Maximum literal length to inline in the log before applying length+hash redaction.
/// Below this, short values like coordinates are written verbatim.
...
/// |-----------------|----------------|---------------------------|
/// | KeyType         | `text`         | `len`, `sha256`           |
/// | ClipboardWrite  | `text`         | `len`, `sha256`, `tag`    |
/// | ClipboardRead   | `data`         | `len`, `sha256`, `tag`    |
/// | OpenUrl         | `url`          | `origin`                  |
...
pub fn redact_action(cap: Capability, mut action: Value) -> Value {
    ...
    match cap {
        Capability::KeyType => {
            redact_string_field(obj, "text");
        }
        Capability::ClipboardWrite => {
            redact_string_field_tagged(obj, "text");
        }
        Capability::ClipboardRead => {
            // ClipboardRead requests carry no sensitive args (just a request).
            // Redaction of the response happens at the outbound boundary.
        }
        Capability::OpenUrl => {
...
/// Replace `field`'s string value with `{len, sha256}`.
fn redact_string_field(obj: &mut serde_json::Map<String, Value>, field: &str) {
    if let Some(Value::String(s)) = obj.get(field) {
        if s.len() > SHORT_VALUE_BYTES {
            let bytes = s.as_bytes();
            let h = sha256_hex(bytes);
            let len = bytes.len();
            obj.remove(field);
            obj.insert(format!("{}_len", field), json!(len));
            obj.insert(format!("{}_sha256", field), json!(h));
        }
    }
}
...
fn redact_string_field_tagged(obj: &mut serde_json::Map<String, Value>, field: &str) {
    if let Some(Value::String(s)) = obj.get(field) {
        ...
        obj.insert(format!("{}_len", field), json!(len));
        obj.insert(format!("{}_sha256", field), json!(h));
        obj.insert(format!("{}_tag", field), json!(tag));
    }
}
...
fn redact_bytes_field(obj: &mut serde_json::Map<String, Value>, field: &str) {
    ...
    obj.remove(field);
    obj.insert(format!("{}_len", field), json!(len));
    obj.insert(format!("{}_sha256", field), json!(hash));
}
...
mod tests {
    ...
    #[test]
    fn keytype_short_passes_through() {
        // Below SHORT_VALUE_BYTES: literal preserved (e.g. "Enter" is auditable as-is)
        let action = json!({"text": "Enter"});
        let r = redact_action(Capability::KeyType, action);
        assert_eq!(r["text"], "Enter");
    }
