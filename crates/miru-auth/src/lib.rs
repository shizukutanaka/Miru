...
/// Derive an AES key from a PIN, device ID, and a per-session nonce using PBKDF2-SHA256.
///
/// `nonce` MUST be fresh random bytes generated by the host for each pairing
/// session and sent to the viewer over the (already authenticated) channel.
/// Without a random nonce, an attacker who knows `device_id` (publicly
/// visible on the signal server) could pre-compute all 10^6 keys offline,
/// then brute-force any captured pairing session in seconds on a GPU.
///
/// First-connection only (one-time use); subsequent uses TOFU pubkey.
pub fn pin_to_key(pin: &str, device_id: &str, nonce: &[u8; 16]) -> [u8; 32] {
    let mut key = [0u8; 32];
    // salt = device_id || nonce — unique per pairing session
    let mut salt = Vec::with_capacity(device_id.len() + 16);
...
mod tests {
    ...
    fn pin_to_key_deterministic() {
        let nonce = [0u8; 16];
        let k1 = pin_to_key("123456", "DEVID", &nonce);
        let k2 = pin_to_key("123456", "DEVID", &nonce);
        assert_eq!(k1, k2);
    }
    ...
    fn pin_to_key_different_pin() {
        let nonce = [0u8; 16];
        let k1 = pin_to_key("123456", "DEVID", &nonce);
        let k2 = pin_to_key("654321", "DEVID", &nonce);
        assert_ne!(k1, k2);
    }
