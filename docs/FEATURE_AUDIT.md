...
| 4 | Wayland / PipeWire キャプチャ | `platform/linux.rs` の `PipeWireCapturer` が `bail!` するのみ。Ubuntu 22.04+ / Fedora の標準セッション(Wayland)で動かない | **交渉部分は実装済み・実ポータルで検証。フレーム取得は未実装** — 完了ではない。項目2と同じく `pipewire` クレートは**使わない**: `csrc/miru_portal.c`(GDBus + libpipewire の C シム)+ `build.rs` + `platform/portal_ffi.rs` で **Rust 依存ゼロ**(`Cargo.toml` から `pipewire = "0.8"` を削除)。CreateSession → SelectSources → Start → OpenPipeWireRemote の全段を実装し、**失敗したステップ名と D-Bus エラー文を型付きで返す**(`PortalError { stage, detail }`)— 「ポータルが拒否した」ではなく「SelectSources が拒否された」と言えることが運用上の差になる。**検証環境をコンテナ内に構築した**: sway(headless)+ pipewire + wireplumber + xdg-desktop-portal(-wlr)を起動し、実ポータル(ScreenCast v5)に対して **CreateSession と SelectSources の成功を確認**(バックエンドが `HEADLESS-1` を選択するところまでログで追跡)。**`Start` はこの構成では失敗する**: xdpw が `unable to receive a valid format from wlr_screencopy` を出す — DRM デバイスの無い headless sway が提示するピクセルフォーマットを xdpw が受け付けないためで、**クライアント側の欠陥ではない**(同じ環境で `grim` は screencopy 経由で実際にスクリーンショットを取得できる)。 | **フレーム取得も実装・実検証済み**: `PipeWireVideoStream` が node へ接続し、spa_pod でフォーマット交渉(BGRx 優先 + 5候補)→ バッファを dequeue → **latest-wins の単一スロット**へコピー(ADR 0013 の映像ドロップ方針と同一 — リモートデスクトップは最新フレームのみ要る)。バッファが小さい場合は必要サイズを返して呼び出し側が拡張・再試行するためフレームを落とさない。**検証方法**: 外部ソースやコンポジタに依存しないよう、テスト自身が PipeWire プロデューサ(`csrc/miru_pw_testsrc.c`、**build.rs はコンパイルしない = 出荷物に含めない**)を起動し、実 PipeWire デーモン越しに **320x240 BGRx のフル解像度フレーム3枚**を受信することを確認。実装中に2件の実バグを検出・修正: `SPA_POD_CHOICE_ENUM_Id` の個数が既定値を含む点を誤り segfault、プロデューサの `PW_STREAM_FLAG_DRIVER` により誰もクロックせずフレームがゼロ。**`LinuxCapturer` への配線も完了**: `PipeWireCapturer` の `bail!` スタブを実装で置換。session を保持(drop で grant 失効)、`next_frame` はフォーマット未確定なら `None`、バイト数不足なら**エンコーダに範囲外読み取りをさせず bail**、Bgra32 の `RawFrame` を返す。`displays()` は**1枚だけ**返す — ポータルはユーザーが自分のピッカーで選んだ1ソースしか許可しないため、複数を広告すると `select_display` が応えられない選択肢をビューアに見せることになる。**残**: `Start` が通る実機 Wayland での portal → stream → capture 通し確認のみ(本サンドボックスの headless sway は DRM 非搭載のため xdpw がフォーマットを拒否する) |
| 5 | macOS 60fps キャプチャ | `crates/miru-capture/src/platform/macos.rs` — CGDisplayCreateImage ポーリングで ~20fps。**「未着手」ではなく「動作する低速経路がある」** — 全 macOS で動き、Objective-C 連携も不要 | **現状の実装を清掃済み、SCK 移行は macOS 実機必須で未実施**。コード検分で3点発見し修正: (a) **潜在バグ** — `next_frame()` は `stream_state.latest_frame` が `Some` なら返し続け**消費もクリアもしない**。書き込み側が無いため現在は不発だが、SCK を実装して1枚でも書いた瞬間に**同一フレームを永久に返し画面が固まる**。実装者が最初に踏む地雷だった (b) **存在しない機能のための機構** — 書き込み側の無い `Arc<Mutex<StreamState>>` に毎フレーム lock を払い、`start_cg_stream()` は起動毎に「stream init pending」と warn するだけの no-op、`StreamState.width/height` は書かれるが読まれない。計57行を削除 (201→144行) (c) **虚偽ドキュメント** — 冒頭が「macOS 14+: ScreenCaptureKit — IOSurface, GPU-resident」と**実装済みかのような戦略表**を掲げていた(規約「未実装機能を実装済みとして文書化しない」違反)。実態(ポーリング ~20fps、60fps には SCK 移行が必要)に書き換え | **SCK 本体は意図的に書かない**。Objective-C 連携は macOS 上でしか記述も検証もできず、**動作中の経路を検証不能なコードで置き換えるのは現ユーザーへの回帰リスク**。本ブランチの一貫方針(検証できないものは書かない)に従う。規模: 中(macOS 実機で 1-2週) |
...
| 17 | ~~マルチモニタの座標指定が3OS全てで壊れている~~ → **修正済み** | viewer は「表示中ディスプレイ内の正規化座標」を送るが、**3バックエンドすべてが「プライマリ基準の正規化」として解釈**していた。加えて `DisplayInfo` に x/y が無く**プロトコル上マルチモニタ配置を表現できなかった** | **修正済**。①`DisplayInfo` に `x`/`y`(`serde(default)`)、`MouseDown`/`MouseUp`/`Scroll` に `display`(同)を追加 — クリックはディスプレイ情報を全く持っておらず、移動が直っても解決不能だった。②`miru-common/src/display_map.rs` を新設(純粋関数・テスト9件): 負オフセット・インデックス不正時のプライマリfallback・ゼロ除算回避を含む。③変換は**ホスト側 `InputHandler`** で実施 — `miru_input::inject` はディスプレイ情報を持たない自由関数であり、かつ **`input_handler.rs` はオフラインゲートが実コンパイル・実行**するため変換の適用まで検証できる。レイアウト未知なら**素通し**(推測で動かさない)。④Windows は `MOUSEEVENTF_VIRTUALDESK` 併用、macOS は `screen_point()` を全ディスプレイ bounds の和集合基準に変更、**Linux は変更不要**(uinput ABS が既に仮想デスクトップ全体)。⑤キャプチャ側が実オフセットを報告(X11 の `MonitorGeometry` は元から x/y を持っていたが捨てていた)。⑥ビューアが `display` を送出 — **従来は送信ペイロードに `display` が存在せず**、ディスプレイ切替は映像だけ切り替わり入力は常にプライマリへ行っていた | **退行懸念への回答**: 監査は「VIRTUALDESK 単独追加は通常ケースを動かす退行」として見送っていた。正しい。**鍵は変換が『1枚・原点配置なら恒等写像』であること**で、これを `display_map` と `input_handler` の両方でテスト固定した。**残**: OS 注入/キャプチャは本環境ではパースのみ — `cargo test --workspace` と実機マルチモニタでの確認が必要 |
