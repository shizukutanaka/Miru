...
| 15h | タッチで右クリックできない | 1本指=左クリック、2本指=スクロールのみで、**右クリックの手段が存在しない**。比較実装(Guacamole / RustDesk モバイル / OS 標準のタッチ操作)はいずれも長押しか2本指タップを右クリックに割り当てている | **未着手(意図的)**。2本指タップへの割当は現状の設計では実現できない — 1本目の `touchstart` が**即座に左ボタンを押す**ため、2本目が着地した時点で左クリックは既にホストへ送信済みになり、「右クリックのつもりが左クリックも発火する」誤動作になる。正しくは**ジェスチャが確定するまで初回押下を遅延**させる設計だが、ドラッグの応答遅延というトレードオフがあり実機での確認が必要。規模: 中 |
