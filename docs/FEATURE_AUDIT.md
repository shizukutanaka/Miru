...
| 10 | UI アクセシビリティ欠如 | 商用品質監査(2026-07)で確認: 12個の `.tsx` 全体で `aria-*`/`role=` 使用が実質ゼロだった。対応済み(7/12): `ConnectScreen.tsx`、`PairingDialog.tsx`、`SessionScreen.tsx`、`OnboardingWizard.tsx`、`AuditLogViewer.tsx`、`DisplayTabs.tsx`、`App.tsx`(nav の `aria-current`/`aria-label`)。いずれも `tsc` 検証済み | 残り: `ConstellationMap`/`AgentTokenIssue`/`TimelineScrubber`(`PairPrompt` は項目12参照)。規模: 小(コンポーネントごと) |
