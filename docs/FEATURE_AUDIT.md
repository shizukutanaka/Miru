...
|---|------|---------------------------|------------------|
| 1 | ホスト側システム音声キャプチャ | `crates/miru-host/src/` 内のどこにも `AudioEncoder` の呼び出しが無く、`AudioFrame` が一度も送信されない。Opus エンコード/デコード/再生 (`crates/miru-audio/src/lib.rs`) とコーデック交渉 (`crates/miru-common/src/codec.rs` の `negotiate_audio`) は完成済みだが宙に浮いている。`crates/miru-host/src/session.rs` の `Features.audio` は虚偽広告を避けるため `false` に修正済み | 実装方針を外部調査で具体化済み (`docs/RESEARCH_NOTES.md` §5): `cpal` を第一候補にすると3OSを1 APIで覆える可能性が高い — macOS 14.6+ の CoreAudio loopback は cpal 対応済み、Linux は PulseAudio の `<sink>.monitor` ソースをデバイスとして列挙、Windows は WASAPI loopback (`AUDCLNT_STREAMFLAGS_LOOPBACK`) だが **EVENTCALLBACK と併用不可の既知の罠あり**(cpal 未対応ならポーリング型で `wasapi` crate 補完)。`miru-audio` の Opus encoder は完成済みなので「OS音声 → f32 interleaved」のキャプチャ層のみが必要。規模: 中 |
...
| 10 | UI アクセシビリティ欠如 | 商用品質監査(2026-07)で確認: 12個の `.tsx` 全体で `aria-*`/`role=` 使用が実質ゼロだった。対応済み: `ConnectScreen.tsx`(label/input 関連付け、`role="alert"`、ピア接続ボタンの `aria-label`)、`PairingDialog.tsx`(`role="dialog"` + `aria-modal`)、`SessionScreen.tsx`(canvas の `aria-label`、統計/警告バナーの `role="status"`/`"alert"` + `aria-live`)、`OnboardingWizard.tsx`(ステップ進捗を `role="progressbar"` 化)。いずれも `tsc` 検証済み | 残り8コンポーネント(`AuditLogViewer`/`ConstellationMap`/`AgentTokenIssue`/`TimelineScrubber`/`DisplayTabs`/`PairPrompt`/`App.tsx` 等)が未着手。規模: 小(コンポーネントごと) |
