...
| 8 | ホスト側 TOFU が無条件自動承認(PIN 検証なし) | `crates/miru-host/src/session.rs` 735-765行目: `TrustDecision::Unknown` の場合、PIN 照合を一切行わず `warn!` ログを出すだけで即座に `acl.trust(...)` して自動承認している。コメントに「v0.3 will add a confirmation dialog」と明記されており、この設計上のギャップは host 開発チーム自身が認識・先送りしていたもの。本セッションで実装した viewer 側ゲート(項目7)はこの欠落を viewer 側でのみ埋めたもので、host 側は未対応のまま | host 側にも同様の確認フロー(ヘッドレス実行時は Tauri IPC が無いため、CLI プロンプトか設定ファイルでの事前承認リストが必要)。規模: 小〜中 |
